...
void LCD_sendCommand(uint8 command){
	...
	systicktimerDelay(1);       /*delay for tas = 50ns*/
	GPIO_writePin(LCD_EN_PORT_ID ,LCD_EN_PIN_ID,LOGIC_HIGH);
	systicktimerDelay(1);    /*delay  process for tps-tdsw=290-190=100*/
#if(LCD_BIT_DATA_MODE == 4)
	GPIO_writePin(LCD_DATA_PORT_ID,LCD_DB4_PIN_ID,GET_BIT(command,4));
	GPIO_writePin(LCD_DATA_PORT_ID,LCD_DB5_PIN_ID,GET_BIT(command,5));
	GPIO_writePin(LCD_DATA_PORT_ID,LCD_DB6_PIN_ID,GET_BIT(command,6));
	GPIO_writePin(LCD_DATA_PORT_ID,LCD_DB7_PIN_ID,GET_BIT(command,7));
	systicktimerDelay(1);   /*delay for tdsw=100ns*/
	GPIO_writePin(LCD_EN_PORT_ID ,LCD_EN_PIN_ID,LOGIC_LOW);
	systicktimerDelay(1);       /*delay for th=13ns */
	GPIO_writePin(LCD_EN_PORT_ID ,LCD_EN_PIN_ID,LOGIC_HIGH);
	systicktimerDelay(1);        /*delay  process for tps-tdsw=290-190=100*/
